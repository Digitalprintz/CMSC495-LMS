...
public class UserChangePass extends JFrame 
{
	...
	public UserChangePass() 
	{
		...
		//verifies passwords match. Also takes a global count to 5. If the count = 5, then it will automatically redirect to the login page
		//for security, in the event that someone is left logged in. It also logs the forceful logout in the logging database.
				...
				else
				{
					try 
					{
						...
						while(rs.next())
						{
							...
							{
								JOptionPane.showMessageDialog(null, "Password does not match account currently logged in. Please try again.");
								count++;
